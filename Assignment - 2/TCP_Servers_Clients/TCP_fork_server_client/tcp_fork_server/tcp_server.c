...
int main(int argc, char **argv)
{
	...
	{  
		...
		else
		{
			// we are in child
			close(sock);
			printf("forking new child process %d to handle the client.. \n",getpid());

			memset(line, 0, MAX_DATA);
			/* Read a line from the client socket ... */
			n = readline(clientfd, line, MAX_DATA);
			if (n <= 0) {
				sprintf(err_msg, "Child: %d - socket read error..\n",getpid());
				//perror("Child: %d socket read error..",getpid());
				exit(-1);
			}
			...
			if(!result)	{			//line matches "test_file_1.txt" 
				...
				if(!result){ 		//line matches "test_file_2.txt"
					...
					printf("Closing client connection. child exiting...\n");
					close(clientfd);
					exit(0);
				}
			} 
